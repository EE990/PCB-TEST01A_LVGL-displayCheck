 ...
 */
 ...
 static esp_err_t panel_xf024qv04b_del(esp_lcd_panel_t *panel);
 static esp_err_t panel_xf024qv04b_reset(esp_lcd_panel_t *panel);
 static esp_err_t panel_xf024qv04b_init(esp_lcd_panel_t *panel);
 static esp_err_t panel_xf024qv04b_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end, int y_end,
                                           const void *color_data);
 static esp_err_t panel_xf024qv04b_invert_color(esp_lcd_panel_t *panel, bool invert_color_data);
 static esp_err_t panel_xf024qv04b_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y);
 static esp_err_t panel_xf024qv04b_swap_xy(esp_lcd_panel_t *panel, bool swap_axes);
 static esp_err_t panel_xf024qv04b_set_gap(esp_lcd_panel_t *panel, int x_gap, int y_gap);
 static esp_err_t panel_xf024qv04b_disp_on_off(esp_lcd_panel_t *panel, bool off);
 static esp_err_t panel_xf024qv04b_sleep(esp_lcd_panel_t *panel, bool sleep);
 ...
 esp_err_t panel_xf024qv04b_bus_SPI_init(size_t buffer_size, const esp_lcd_panel_io_handle_t *lcd_io)
 {
                          ...
                          esp_lcd_panel_handle_t *ret_panel,char *interfasce_str,size_t interfasce_buffer_size)
 {
     ...
     LCD->base.del = panel_xf024qv04b_del;
     LCD->base.reset = panel_xf024qv04b_reset;
     LCD->base.init = panel_xf024qv04b_init;
     LCD->base.draw_bitmap = panel_xf024qv04b_draw_bitmap;
     LCD->base.invert_color = panel_xf024qv04b_invert_color;
     LCD->base.set_gap = panel_xf024qv04b_set_gap;
     LCD->base.mirror = panel_xf024qv04b_mirror;
     LCD->base.swap_xy = panel_xf024qv04b_swap_xy;
     LCD->base.disp_on_off = panel_xf024qv04b_disp_on_off;
     LCD->base.disp_sleep = panel_xf024qv04b_sleep;
     *ret_panel = &(LCD->base);
     ESP_LOGD(TAG, "new LCD:xf024qv04b panel @%p", LCD);
 ...
 static esp_err_t panel_xf024qv04b_del(esp_lcd_panel_t *panel)
 {
     _xf024qv04b_panel_t *LCD = __containerof(panel, _xf024qv04b_panel_t, base);
 //***************** 20250704  新增 *****************
 ...
 //***************** 20250704  新增 *****************
     ...
     ESP_LOGD(TAG, "del LCD:xf024qv04b panel @%p", LCD);
     free(LCD);
     return ESP_OK;
 }
 ...
 static esp_err_t panel_xf024qv04b_init(esp_lcd_panel_t *panel)
 {
     ...
     ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0x11, NULL, 0), TAG,"io tx param failed");//sleep out
     vTaskDelay(pdMS_TO_TICKS(150));
 
     ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0x36, (uint8_t[]) {0x00}, 1), TAG,"io tx param failed");// 翻轉用
     ...
     ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0xB7, (uint8_t[]) {0x06}, 1), TAG,"io tx param failed");//
 
     ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0xBB, (uint8_t[]) {0x30}, 1), TAG,"io tx param failed");
     ...
     ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0xB6, (uint8_t[]) {0x0F}, 1), TAG,"io tx param failed");  
 
     ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0xD0, (uint8_t[]) {0xA7}, 1), TAG,"io tx param failed");     
 
     ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0xD0, (uint8_t[]) {0xA4,0xA1}, 2), TAG,"io tx param failed");     
 
     ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0xB6, (uint8_t[]) {0x01}, 1), TAG,"io tx param failed");     
 
     ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0xE0, (uint8_t[]) {0x0F,0x02,0x07,0x05,0x06,0x14,0x2f,0x54,0x46,0x38,0x13,0x11,0x2E,0x35}, 14), TAG,"io tx param failed");
 
     ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0xE1, (uint8_t[]) {0x0F,0x08,0x0C,0x0C,0x09,0x05,0x2F,0x43,0x46,0x36,0x10,0x12,0x2C,0x32}, 14), TAG,"io tx param failed");  
     ...
     ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0x2A, (uint8_t[]) {0x00,0x00,0x00,0xF0}, 4), TAG,"io tx param failed");//Column Address Set ,0xf0=240
 
     ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0x2B, (uint8_t[]) {0x00,0x00,0x01,0x40}, 4), TAG,"io tx param failed");//Row Address Set ,0x140=320  
 ...
 static esp_err_t panel_xf024qv04b_disp_on_off(esp_lcd_panel_t *panel, bool on_off)
 {
     _xf024qv04b_panel_t *LCD = __containerof(panel, _xf024qv04b_panel_t, base);
     esp_lcd_panel_io_handle_t io = LCD->io;
     int command = 0;
